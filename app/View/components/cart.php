...
$items = new Items();
$items->addShoppingCart("addToCart");
$items->deleteFromShoppingCart("deleteItemCart");

if (isset($_POST["add"])) {
    $itemID = $_POST["itemID"];
    $items->increaseQuantity($itemID, $_SESSION["cart"]);
}

if (isset($_POST["sub"])) {
    $itemID = $_POST["itemID"];
    $items->decreaseQuantity($itemID, $_SESSION["cart"]);
}

if (!empty($_SESSION["cart"])) {
    $_SESSION["totalPrice"] = $items->calculateTotal($_SESSION["cart"]);
} else {
    ...
    <div class="container">
        <section class="pt-2 mx-2">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h3"><span class="fs-2 fw-lighter text-light">Shopping Cart List</span></h1>
            </div>
            <div class="row">
                <div class="col-8">
                    <div class="row g-0 gap-0">
                        <?php if (isset($_SESSION["cart"]) && $_SESSION["cart"] != null) {
                                        ...
                                        <div class="col-md-9">
                                            <div class="card-body position-relative h-100">
                                                ...
                                                <div class="hstack gap-3 bottom-0 position-absolute bottom-0 start-0 ms-2 mb-2">
                                                    ...
                                                    <form action="/public/cart.php" class="small border-start" method="GET">
                                                        <button name="deleteItemCart" value="<?php echo $value["itemID"] ?>" class="btn small bottom-0">Remove</button>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="col-12 py-5">
                                <h1 class=" fs-1 fw-lighter text-light">Your Shopping Cart list is empty.</h1>
                            </div>
                        <?php } ?>
                    </div>
                </div>
